# -*- coding: utf-8 -*-
"""
build_bilingual_engine.py
Constructs a comprehensive, rock-solid, production-grade bilingual (ID / EN) i18n engine
for index.html and injects it seamlessly.
"""
...
def main():
    ...
    js_code = r'''
        // 7. Bilingual Language Switcher (ID / EN) — Full Comprehensive i18n Engine
        ...
        const i18nDictionary = {
          id: {
            ...
            overviewLead: '<strong>Zimbra Link Installer</strong> adalah repositori referensi arsitektural enterprise, indeks arsip biner lengkap, dan utilitas instalasi otomatis untuk <strong>Zimbra Collaboration Suite (ZCS)</strong> dari rilis historis <strong>4.5.x hingga rilis aktif 10.1.x</strong>.',
            overviewUnifies: 'Repositori ini menyatukan:'
          },
          en: {
            title: "Zimbra Link Installer — Binary Archive & CVE Matrix",
            ...
            overviewLead: '<strong>Zimbra Link Installer</strong> is an enterprise architectural reference repository, comprehensive binary archive index, and automated installation utility for <strong>Zimbra Collaboration Suite (ZCS)</strong> spanning historical releases from <strong>4.5.x up to active 10.1.x</strong>.',
            overviewUnifies: 'This repository unifies:'
          }
        };
        ...
        function setLanguage(lang) {
          ...
          const dict = i18nDictionary[lang] || i18nDictionary['id'];

          // 1. Toggle active button styles
          if (lang === 'en') {
          ...
          // 2. Update Skip to Content Link
          const skipLinkEl = document.querySelector('.skip-to-content');
          if (skipLinkEl) skipLinkEl.textContent = dict.skipLink;

          // 3. Update Navbar Links
          const navLinkEls = document.querySelectorAll('.nav-links .nav-link');
          dict.navLinks.forEach((htmlText, idx) => {
            if (navLinkEls[idx]) navLinkEls[idx].innerHTML = htmlText;
          });

          // 4. Update Sidebar Section Titles
          const sidebarTitleEls = document.querySelectorAll('.sidebar-title');
          dict.sidebarTitles.forEach((titleText, idx) => {
            if (sidebarTitleEls[idx]) sidebarTitleEls[idx].textContent = titleText;
          });
          ...
          // 6. Update Section Headings (H2, H3, H4)
          Object.keys(dict.sections).forEach(secId => {
            const el = document.getElementById(secId);
            if (el) {
              el.innerHTML = dict.sections[secId];
            }
          });
          ...
          // 8. Update Table Filter Placeholders
...
'''
    ...
    # Replace the old language switcher block with the comprehensive engine
    ...
    new_html = html[:match.start()] + js_code.strip() + html[match.end():]

    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(new_html)

    print("SUCCESS: index.html updated with comprehensive bilingual i18n engine!")
    return 0
