# -*- coding: utf-8 -*-
"""
build_comprehensive_i18n.py
Generates and injects the complete, production-grade bilingual i18n engine into index.html.
Translates all headers, navigation elements, text descriptions, lists, callouts, and guides.
"""

import json
import re

def generate_i18n_script():
    return r'''
        // =====================================================================
        ...
        const i18nDict = {
          id: {
            ...
            sidebarLinks: {
              ...
              "#techfilesonline-zimbra-foss-101x-ian-walker-builds": '<i class="fa-solid fa-cloud-arrow-down"></i> TechFiles 10.1.x',
              "#zcs-foss-101x-series-all-26-community-releases-20242026": '<i class="fa-solid fa-layer-group"></i> FOSS 10.1.x (26 Builds)',
              "#zcs-foss-100x-series-all-17-community-releases-20232026": '<i class="fa-solid fa-layer-group"></i> FOSS 10.0.x (17 Builds)',
              "#zcs-foss-900x-series-all-8-kepler-community-releases-20202025": '<i class="fa-solid fa-layer-group"></i> FOSS 9.0.0.x (8 Builds)',
              "#zcs-foss-8815x-series-all-joule-community-releases-20182024": '<i class="fa-solid fa-layer-group"></i> FOSS 8.8.15.x',
              "#build-systems--source-compilation-guide": '<i class="fa-solid fa-screwdriver-wrench"></i> Build & Kompilasi',
              "#security-architecture--comprehensive-cve-matrix-20162026": '<i class="fa-solid fa-shield-virus"></i> Matriks CVE & Keamanan',
              "#taxonomy-of-zimbra-threat-vectors--exploit-chains": '<i class="fa-solid fa-diagram-project"></i> Taksonomi Ancaman',
              "#zero-day-emergency-incident-response--hardening-protocol": '<i class="fa-solid fa-kit-medical"></i> Protokol Zero-Day',
              "#enterprise-dns-architecture": '<i class="fa-solid fa-network-wired"></i> DNS & Unbound',
              "#enterprise-hot-backup-and-disaster-recovery": '<i class="fa-solid fa-hard-drive"></i> Hot Backup (zmbackup)',
              "#firewall-network-and-host-hardening": '<i class="fa-solid fa-shield-halved"></i> Firewall & UFW',
              "#operational-best-practices-rfc-2119": '<i class="fa-solid fa-book-bookmark"></i> Praktik Terbaik (RFC 2119)',
              "#strategic-migration--upgrade-methodology": '<i class="fa-solid fa-arrow-right-arrow-left"></i> Migrasi & Z2C',
            ...
            headings: {
              ...
              "enterprise-dns-architecture": '<a href="#enterprise-dns-architecture" class="anchor-link">#</a> Arsitektur DNS Enterprise (Unbound, BIND9, dnsdist & RBL)',
              "sec-unbound-local-recursive-resolver": '<a href="#sec-unbound-local-recursive-resolver" class="anchor-link">#</a> 1. Konfigurasi Caching Recursive Resolver Lokal (Unbound)',
              "sec-postfix-rbl-dnsbl-hardening": '<a href="#sec-postfix-rbl-dnsbl-hardening" class="anchor-link">#</a> 2. Integrasi Multi-Tier RBL / DNSBL pada Postfix MTA',
              "enterprise-hot-backup-and-disaster-recovery": '<a href="#enterprise-hot-backup-and-disaster-recovery" class="anchor-link">#</a> Cadangan Panas & Pemulihan Bencana Enterprise (zmbackup)',
              "firewall-network-and-host-hardening": '<a href="#firewall-network-and-host-hardening" class="anchor-link">#</a> Firewall, Jaringan & Pengerasan Host (UFW, iptables & Fail2Ban)',
              "sec-fail2ban-bruteforce-defense": '<a href="#sec-fail2ban-bruteforce-defense" class="anchor-link">#</a> Konfigurasi Fail2Ban untuk Perlindungan Autentikasi ZCS',
          ...
          en: {
            ...
            sidebarLinks: {
              ...
              "#zcs-foss-900x-series-all-8-kepler-community-releases-20202025": '<i class="fa-solid fa-layer-group"></i> FOSS 9.0.0.x (8 Builds)',
              "#zcs-foss-8815x-series-all-joule-community-releases-20182024": '<i class="fa-solid fa-layer-group"></i> FOSS 8.8.15.x',
              "#build-systems--source-compilation-guide": '<i class="fa-solid fa-screwdriver-wrench"></i> Build & Compilation',
              "#security-architecture--comprehensive-cve-matrix-20162026": '<i class="fa-solid fa-shield-virus"></i> Security & CVE Matrix',
              "#taxonomy-of-zimbra-threat-vectors--exploit-chains": '<i class="fa-solid fa-diagram-project"></i> Threat Taxonomy Flow',
              "#zero-day-emergency-incident-response--hardening-protocol": '<i class="fa-solid fa-kit-medical"></i> Zero-Day Protocol',
              "#enterprise-dns-architecture": '<i class="fa-solid fa-network-wired"></i> DNS & Unbound',
              "#enterprise-hot-backup-and-disaster-recovery": '<i class="fa-solid fa-hard-drive"></i> Hot Backup (zmbackup)',
              "#firewall-network-and-host-hardening": '<i class="fa-solid fa-shield-halved"></i> Firewall & UFW',
              "#operational-best-practices-rfc-2119": '<i class="fa-solid fa-book-bookmark"></i> RFC 2119 Best Practices',
              "#strategic-migration--upgrade-methodology": '<i class="fa-solid fa-arrow-right-arrow-left"></i> Migration & Z2C',
            ...
            headings: {
              ...
              "enterprise-dns-architecture": '<a href="#enterprise-dns-architecture" class="anchor-link">#</a> Enterprise DNS Architecture (Unbound, BIND9, dnsdist & RBL)',
              "sec-unbound-local-recursive-resolver": '<a href="#sec-unbound-local-recursive-resolver" class="anchor-link">#</a> 1. Local Caching Recursive Resolver Configuration (Unbound)',
              "sec-postfix-rbl-dnsbl-hardening": '<a href="#sec-postfix-rbl-dnsbl-hardening" class="anchor-link">#</a> 2. Multi-Tier RBL / DNSBL Integration on Postfix MTA',
              "enterprise-hot-backup-and-disaster-recovery": '<a href="#enterprise-hot-backup-and-disaster-recovery" class="anchor-link">#</a> Enterprise Hot Backup & Disaster Recovery (zmbackup)',
              "firewall-network-and-host-hardening": '<a href="#firewall-network-and-host-hardening" class="anchor-link">#</a> Firewall, Network & Host Hardening (UFW, iptables & Fail2Ban)',
              "sec-fail2ban-bruteforce-defense": '<a href="#sec-fail2ban-bruteforce-defense" class="anchor-link">#</a> Fail2Ban Jail Configuration for ZCS Authentication Defense',
        ...
        function setLanguage(lang) {
          ...
          Object.keys(dict.sidebarLinks).forEach(href => {
            ...
            if (link) link.innerHTML = dict.sidebarLinks[href];
          });

          // 6. Update Table Search Filter Placeholders
          document.querySelectorAll('.table-filter').forEach(input => {
            input.placeholder = dict.filterPlaceholder;
          });

          // 7. Update Copy Buttons
          document.querySelectorAll('.copy-btn').forEach(btn => {
            if (!btn.classList.contains('copied')) {
              btn.innerHTML = dict.copyBtnText;
            }
          });
        }
        ...
        // Initialize from localStorage or default to Indonesian
...
def main():
    ...
    # Find the section containing the i18n script
    script_start_marker = "// 7. Comprehensive Full-Document Bilingual i18n Engine (ID <-> EN)"
    script_pos = html.find(script_start_marker)
    if script_pos != -1:
        # Find closing tag </script>
        script_end_pos = html.find('</script>', script_pos)
        prefix = html[:script_pos - 8] # backtrack to // =====
        suffix = html[script_end_pos:]
        new_script = generate_i18n_script()
        html = prefix + new_script.strip() + "\n      " + suffix
        print("Updated existing i18n script in index.html")
    else:
        print("Marker not found, please check script block in index.html")
        return 1

    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)

    print("SUCCESS: Comprehensive i18n dictionary synchronized!")
    return 0
